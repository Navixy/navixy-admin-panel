...
L.Symbol.Marker = L.Class.extend({
    ...
    initialize: function (options) {
        L.Util.setOptions(this, options);
        this.options.markerOptions.clickable = this.options.markerOptions.clickable || false;
	...
	if(!this.options.rotate) {
            marker = new L.Marker(directionPoint.latLng, this.options.markerOptions);
        }
        else {
            this.options.markerOptions.angle = directionPoint.heading;
            marker = new L.RotatedMarker(directionPoint.latLng, this.options.markerOptions);
        }

	if(this.options.markerOptions.onclick){
		var me=this,fn=function(){
		  return me.options.markerOptions.onclick.apply(me.options.markerOptions.scope||this,arguments);
		};
		
		marker.on('click',fn);
		marker.on('remove',function(){marker.off('click',fn);});
	}
