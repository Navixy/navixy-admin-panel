/**
 * @class Base.plugins.FieldPostfix
 * @extends Ext.Base
 */
...
Ext.define('NavixyPanel.plugins.FieldPostfix', {
    ...
    createPostfixEl: function (field) {
        ...
        if (this.units) {
            postfixEl.innerText = Ext.util.Format.mUnits(field.value, this.units);
        } else {
    ...
    changeFn: function (field, newVal) {
        ...
        if (el) {
            try {
                if (newVal) {
                    var left = this.textMetrics.getWidth(newVal) + 10;

                    if (this.units) {
                        el.update(Ext.util.Format.mUnits(newVal, this.units));
                    } else {
